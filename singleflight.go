...
type call[K comparable, T any] struct {
	wg sync.WaitGroup
...
type Group[K comparable, T any] struct {
	...
	m  map[K]*call[K, T] // lazily initialized
}
...
func (g *Group[K, T]) DoChan(key K, fn func() (T, error)) <-chan Result[T] {
	...
	c := &call[K, T]{chans: []chan<- Result[T]{ch}}
	c.wg.Add(1)
	g.m[key] = c
	g.mu.Unlock()
...
func (g *Group[K, T]) doCall(c *call[K, T], key K, fn func() (T, error)) {
	normalReturn := false
	recovered := false
